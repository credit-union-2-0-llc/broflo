...
- **Production api:** internal-only, not directly reachable from outside the web app (Azure App Service)
...
| Cloud | Azure (App Service, Key Vault, Container Registry) |
...
| `PRICE_WATCH_ENABLED` | `false` (off) | Daily price-drop/restock re-check of selected-but-not-yet-ordered gift suggestions (Exa API costs) |
