...
type OrderWithPerson = Order & { person: { name: string } };

const ACTIVE_STATUSES = ["ordered", "processing", "shipped"];

interface OrdersListClientProps {
  initialOrders: OrderWithPerson[];
  initialTotal: number;
}

export function OrdersListClient({ initialOrders, initialTotal }: OrdersListClientProps) {
  const { data: session } = useSession();
  const [orders, setOrders] = useState<OrderWithPerson[]>(initialOrders);
  ...
  useEffect(() => {
    // The server already fetched page 1 / all / newest-first before this
    // component ever mounted — skip the redundant first fetch and only
    // hit the API again once the user actually changes filter/sort/page.
